...
async function main(){
 const pin=process.env.ADMIN_PIN||"1234",pinHash=await bcrypt.hash(pin,12);
 ...
 await prisma.user.upsert({where:{id:"admin"},update:{pinHash,role:Role.ADMIN,active:true},create:{id:"admin",name:"Administrator",pinHash,role:Role.ADMIN}});
}
